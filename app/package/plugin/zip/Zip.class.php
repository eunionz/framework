...
class Zip extends \com\eunionz\core\Plugin
{
    ...
    public function GetFileList($dir, $pref = '')
    {
        $file = array();
        if (file_exists($dir)) {
            $args = func_get_args();
            ...
            while ($files = readdir($dh)) {
                if ($files != "." && $files != "..") {
                    if (is_dir($dir . $files)) {
                        if (strpos($dir . $files, 'ueditor')) {
                            $file = array_merge($file, $this->GetFileList($dir . $files . '/', "$pref$files/"));
                        }
                    } else {
                        $file[] = $pref .$files;
                    }
    ...
    // 解压文件
    public function unZip($dir, $zipfilename)
    {
        exec('unzip -o -d ' . $dir . ' ' . $zipfilename);
        return 1;
    }

    public function shellZip($dir, $zipfilename)
    {
        $curdir = getcwd();
        chdir($dir);
