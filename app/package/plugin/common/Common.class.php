...
class Common extends \cn\eunionz\core\Plugin
{
    ...
    public function get_scheme(){
        ...
        if(empty($scheme)){
            $https_scheme = isset( $_SERVER['HTTPS'] ) ? $_SERVER['HTTPS'] : '';
    ...
    public function get_security_level(array $data)
    {
        ...
        $arr = $this->getConfig('params', 'SECURITY_LEVELS');
     ...
     */
      function get_ip(){
           ...
           $tip = (empty($ip)) ? (isset($_SERVER['REMOTE_ADDR'])?$_SERVER['REMOTE_ADDR']:'127.0.0.1') : $ip;
    ...
    public function get_per_face($bas_uc_uid,$per_face){
        if(defined('UC_AVATAR_PATH')){
            ...
            if($this->getConfig('UPLOAD_PREFIX_URL')){
                //远程上传图片模式
                if(!$this->remote_file_exists($this->getConfig('UPLOAD_PREFIX_URL') . '/runtime/' . $this->getConfig('app','UPLOAD_PATH') . '/' . $per_face)){
                    return  APP_PATH . 'images/headda.gif';
                }else{
                    return  $this->getConfig('UPLOAD_PREFIX_URL') . '/runtime/' . $this->getConfig('app','UPLOAD_PATH') . '/' . $per_face;
                }

            }else{
                //本地上传图片模式
                if(!is_file(APP_RUNTIME_REAL_PATH . $this->getConfig('app','UPLOAD_PATH') . APP_DS . $per_face) || !file_exists(APP_RUNTIME_REAL_PATH . $this->getConfig('app','UPLOAD_PATH') . APP_DS . $per_face)){
                    return  APP_PATH . 'images/headda.gif';
                }else{
                    return  APP_PATH .  'runtime/' . $this->getConfig('app','UPLOAD_PATH') . '/' . $per_face;
                }
            }
    ...
    public function check_mobile_checkcode($mobile,$checkcode){
        ...
        $MOBILE_CHECKCODE_TIMEOUT=$this->getConfig('params','MOBILE_CHECKCODE_TIMEOUT');
        if(isset($_SESSION['s'.$mobile])&&is_array($_SESSION['s'.$mobile])){
            if(time()-$_SESSION['s'.$mobile]['time']>=$MOBILE_CHECKCODE_TIMEOUT){
                if(isset($_SESSION['s'.$mobile])){unset($_SESSION['s'.$mobile]);}
                return false;
            }
            if($checkcode!=$_SESSION['s'.$mobile]['checkcode']){
                return false;
            }
            return true;
        }
    ...
    public function remote_file_exists($remote_file_path){
        $rs=false;
        try{
            $UPLOAD_REOMTE_FTP_SERVER = $this->getConfig('app','UPLOAD_REOMTE_FTP_SERVER');
            if($UPLOAD_REOMTE_FTP_SERVER){
                ...
                @fclose($fp);
            }else if($this->getConfig('app','UPLOAD_REOMTE_SHARE_DIR')){
                $UPLOAD_REOMTE_SHARE_DIR=$this->getConfig('app','UPLOAD_REOMTE_SHARE_DIR');
                if(strpos($UPLOAD_REOMTE_SHARE_DIR,'/')===false){
                    $UPLOAD_REOMTE_SHARE_DIR = APP_REAL_PATH . $UPLOAD_REOMTE_SHARE_DIR;
                }
                return file_exists($UPLOAD_REOMTE_SHARE_DIR .$this->getConfig('app','UPLOAD_REMOTE_INIT_DIR') . '/' . basename($remote_file_path));
            }else{
    ...
    public function write_redirect($url, $is_exit = true,$is_ajax=false,$is_output_ajax=false,$is_changehash=false)
    {
        ...
        $this->loadCore('log')->write(APP_ERROR,'$is_changehash='.$is_changehash,"add_goods_fun");
        if($is_changehash){
        ...
        $this->loadCore('log')->write(APP_ERROR,$rs,"add_goods_fun");
        if($is_output_ajax){
...
//            return false;
    ...
    public function formatPrice($price){
        $SITE_CURRENCY_FORMAT=$this->loadService('shop_params')->get_value_by_key('SITE_CURRENCY_FORMAT',$this->getConfig('shop','SHOP_ID'));
    ...
    public function getImageUrl($imagepath,$image_type='',$is_enable_cdn=true,$is_upload_prefix=true)
    {
        ...
        if (!($this->startsWith($imagepath, 'http://') || $this->startsWith($imagepath, 'https://'))) {
            $imagepath = str_replace(APP_SITE_TEMP_PATH . '/public_html', "", $imagepath);
        }
        $imagepath = str_replace("\\", "/", $imagepath);
        if ($image_type) $image_type = '_' . $image_type . '.jpg';
        $APP_CDN_SITE_DOMAIN_URL = $is_enable_cdn ? $this->getConfig('app', 'APP_CDN_SITE_DOMAIN_URL') : '';
        $UPLOAD_PREFIX_URL = $this->getConfig('app', 'UPLOAD_PREFIX_URL');
        if ($UPLOAD_PREFIX_URL && $is_upload_prefix) {
            $url_prefix = ($this->getConfig('app', 'UPLOAD_PREFIX_URL') ? $this->getConfig('app', 'UPLOAD_PREFIX_URL') : $this->getDomain()) . APP_PATH . APP_SITE_TEMP_PATH . '/public_html';
        } else {
            if($APP_CDN_SITE_DOMAIN_URL){
                $url_prefix = $APP_CDN_SITE_DOMAIN_URL . '/' . $this->getConfig('shop', 'SHOP_ID') . str_ireplace('.', '_', $this->getConfig('params', 'DEFAULT_ADMIN_DOMAIN_SUFFIX')) . '/public_html/runtime/uploads';
            }else{
                $url_prefix = $this->getDomain() . APP_PATH . APP_SITE_TEMP_PATH . '/public_html';
            }
        }
        if (strpos($imagepath, '/') !== false) {
            if (empty($UPLOAD_PREFIX_URL) && $APP_CDN_SITE_DOMAIN_URL) {
                $url_prefix = $APP_CDN_SITE_DOMAIN_URL . '/' . $this->getConfig('shop', 'SHOP_ID') . str_ireplace('.', '_', $this->getConfig('params', 'DEFAULT_ADMIN_DOMAIN_SUFFIX')) . '/public_html';
            }
            if ($this->startsWith($imagepath, 'http://') || $this->startsWith($imagepath, 'https://')) {
    ...
    {
        if (empty($imagepath)) return '';
        $APP_CDN_SITE_DOMAIN_URL = $this->getConfig('app', 'APP_CDN_SITE_DOMAIN_URL');
        if($APP_CDN_SITE_DOMAIN_URL){
    ...
    function shortUrl($url) {
        $access_domain = 'http://' . $_SERVER['HTTP_HOST'];
    ...
    public  function get_head_img($img_name){
        ...
        return $img_name? $this->getImageUrl(ltrim(rtrim($img_dir,'/') .$img_name,'/')):"http://" . $_SERVER['SERVER_NAME']."/images/avatar.png";
    }
...
//        }
    ...
    public function get_admin_tmp_host(){
        return $this->getConfig('shop','SHOP_ID').$this->getConfig('params','DEFAULT_ADMIN_DOMAIN_SUFFIX');
    }
    ...
    public function replace_conent_temp_host($content){
        ...
        if(empty($APP_CDN_SITE_DOMAIN_URL)){
            //未启用CDN加速
            return str_ireplace($this->get_admin_tmp_host(),$_SERVER['HTTP_HOST'],$content);
        }else{
            //启用了CDN加速
...
//        return $this->loadPlugin('crypt')->decrypt(base64_decode($en_str));
    ...
    public function message_url_2_wapurl($content,$client_type='web'){
        ...
        if (!$arr) {
            $site_pc_url = 'http://' . $this->SHOP_ID . $this->getConfig('params', 'DEFAULT_WWW_DOMAIN_SUFFIX');
            $site_wap_url = 'http://' . $this->SHOP_ID . $this->getConfig('params', 'DEFAULT_MOBILE_DOMAIN_SUFFIX');
            $api_host_url = 'http://' . $this->SHOP_ID . $this->getConfig('params', 'DEFAULT_ADMIN_DOMAIN_SUFFIX');
        } else {
            $site_pc_url = 'http://' . $arr['main_domain'];
            $site_wap_url = $arr['mobile_host'];
            $api_host_url= $arr['api_host'];
        }
        $APP_DISTRIBUTER_url_2_wap_urls=$this->getConfig('global','APP_DISTRIBUTER_url_2_wap_urls');
        if($client_type=='web'){
            //pc
    ...
    public function update_app_config_versions($versions){
        sort($versions);
        $content=file_get_contents(PACKAGE_APP_ROOT . "shop/package/config/app.config.php");
        ...
        return file_put_contents(PACKAGE_APP_ROOT . "shop/package/config/app.config.php",$content)?true:false;


    }
    public function addemotion($content='') {
        $host=intval($_SESSION['PLATFORM_SHOP_ID']).$this->getConfig('params','DEFAULT_ADMIN_DOMAIN_SUFFIX');
    ...
    public function get_weapp_info($app_ids,$app_secrets,$curr_app_id=null){
        ...
        if(is_null($curr_app_id)){
            $curr_app_id = (isset($_SESSION['weapp_app_id']) && $_SESSION['weapp_app_id'])?$_SESSION['weapp_app_id']:'';
        }
