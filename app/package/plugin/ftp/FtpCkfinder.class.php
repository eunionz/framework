...
/**
 * EUnionZ PHP Framework FtpCkfinder Plugin class
 * Created by PhpStorm.
...
class FtpCkfinder extends \com\eunionz\core\Plugin
{
    ...
    public function __construct($UPLOAD_REOMTE_SHARE_DIR='',$FTP_HOST='',$FTP_PORT='',$FTP_USER='',$FTP_PASS='',$INIT_DIR='')
    {
        ...
        if($UPLOAD_REOMTE_SHARE_DIR){
            $this->mode=1;
            $this->share_dir=$UPLOAD_REOMTE_SHARE_DIR;
            if(strpos($this->share_dir,'/')===false){
                $this->share_dir = APP_REAL_PATH . $this->share_dir;
            }
        ...
        switch($this->mode){
            ...
            case 1://共享文件夹模式，将$path所对应的文件复制到当前文件夹，
                ...
                if($this->startsWith($newpath,'/')){
                    $rs=@copy($path,$this->curr_dir_path . $newpath);
                }else{
                    $rs=@copy($path,$this->curr_dir_path . '/' . $newpath);
                }
     ...
     */
    public function up_files($paths,$del_local_file=false)
    {
        $arr=array();
        ...
        switch($this->mode){
            ...
            case 1://共享文件夹模式，将$paths所对应的所有文件以文件名本身复制到当前文件夹下，
                foreach($paths as $index=>$path){
                    $arr[$index] = @copy($path,$this->curr_dir_path . '/' . basename($path));
            ...
            case 2://FTP模式，进入ftp根目录下指定目录
                if($this->conn_id){
                    foreach($paths as $index=>$path){
                        $arr[$index] = @ftp_put($this->conn_id,$this->curr_dir_path . '/' .basename($path),$path,FTP_BINARY);
                        if($arr[$index] && $del_local_file){//上传成功且要删除本地文件
                            @unlink($path);
                        }
                    }
                    ...
                    foreach($paths as $index=>$path){
                        if($path){
                            if($this->startsWith($path,'/')){
                                $arr[$index] = @unlink($this->curr_dir_path . $path);
                            }else{
                                $arr[$index] = @unlink($this->curr_dir_path . '/' . $path);
                            }
                        }else{
                ...
                case 2://FTP模式，进入ftp根目录下指定目录
                    if($this->conn_id){
                        foreach($paths as $index=>$path){
                            if($path){
                                if($this->startsWith($path,'/')){
                                    $arr[$index] = @ftp_delete($this->conn_id,$this->curr_dir_path . $path);
                                }else{
                                    $arr[$index] = @ftp_delete($this->conn_id, $this->curr_dir_path .'/' . $path);
                                }
    ...
    public function dir_mkdirs($path)
    {
        ...
        switch($this->mode){
            ...
            case 1://共享文件夹模式
                if($this->startsWith($path,'/')){
                    return @mkdir($this->curr_dir_path  . $path,0777,true);
                }else{
                    return @mkdir($this->curr_dir_path  .  '/' . $path,0777,true);
                }
                break;
