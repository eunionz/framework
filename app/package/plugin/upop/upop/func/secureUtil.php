...
// 初始化日志
$log = new PhpLog ( \SDKConfig::$SDK_LOG_FILE_PATH, "PRC", \SDKConfig::$SDK_LOG_LEVEL );
/**
 * 签名
 *
...
function sign(&$params) {
    ...
    // 签名证书路径
    $cert_path = \SDKConfig::$SDK_SIGN_CERT_PATH;
...
function getPulbicKeyByCertId($certId) {
    // 证书目录
    $cert_dir = \SDKConfig::$SDK_VERIFY_CERT_DIR;
...
function getCertId($cert_path) {
    ...
    openssl_pkcs12_read ( $pkcs12certdata, $certs, \SDKConfig::$SDK_SIGN_CERT_PWD );
    $x509data = $certs ['cert'];
...
function getSignCertId() {
    ...
    return getCertId ( \SDKConfig::$SDK_SIGN_CERT_PATH );
}
function getEncryptCertId() {
    // 签名证书路径
    return getCertIdByCerPath ( \SDKConfig::$SDK_ENCRYPT_CERT_PATH );
}
...
function getPrivateKey($cert_path) {
    ...
    openssl_pkcs12_read ( $pkcs12, $certs, \SDKConfig::$SDK_SIGN_CERT_PWD );
    return $certs ['pkey'];
}
...
function encryptPan($pan) {
    $cert_path = \SDKConfig::$SDK_ENCRYPT_CERT_PATH;
