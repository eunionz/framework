...
include_once APP_REAL_PATH . 'package/plugin/upop/func/PublicEncrypte.php';
include_once APP_REAL_PATH . 'package/plugin/upop/func/common.php';
...
function getCertId($cert_path) {
	...
	openssl_pkcs12_read ( $pkcs12certdata, $certs, MPI_SIGN_CERT_PWD );
	$x509data = $certs ['cert'];
...
function getSignCertId() {
	// 签名证书路径
	return getCertId ( MPI_SIGN_CERT_PATH );
}
function getEncryptCertId() {
	// 签名证书路径
	return getCertIdByCerPath ( MPI_ENCRYPT_CERT_PATH );
}
...
function getPrivateKey($cert_path) {
	...
	openssl_pkcs12_read ( $pkcs12, $certs, MPI_SIGN_CERT_PWD );
	return $certs ['pkey'];
}
