...
//echo 'Retrieve from cache',PHP_EOL;
    ...
    private static function _convertMatrixReferences($formula) {
        ...
        if (strpos($formula,'{') !== FALSE) {
            ...
            $obj=new PHPExcel_Calculation();
            if ($openCount < $closeCount) {
                if ($openCount > 0) {
                    return $obj->_raiseFormulaError("Formula Error: Mismatched matrix braces '}'");
                } else {
                    return $obj->_raiseFormulaError("Formula Error: Unexpected '}' encountered");
                }
