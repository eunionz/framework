...
require_once PACKAGE_APP_ROOT .  'shop/package/plugin/alipay/aop/AopEncrypt.php';
include_once  PACKAGE_APP_ROOT .  'shop/package/plugin/alipay/aop/SignData.php';
include_once  PACKAGE_APP_ROOT .  'shop/package/plugin/alipay/lotusphp_runtime/Logger/Logger.php';
			...
			if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {
				...
				if (!$checkResult) {
					...
					if (strpos($signData->signSourceData, "\\/") > 0) {
					    ...
					    $arr= json_decode($signData->signSourceData,true);
					    if(strtolower($arr['msg'])!='success'){
					        if(isset($arr['sub_msg'])){
                                throw new \Exception($arr['sub_msg']);
                            }else{
