...
class nusoap_server extends nusoap_base {
	...
	*/
	function nusoap_server($wsdl=false){
		...
		if (isset($SERVER)) {
			...
			$this->appendDebug($this->varDump($SERVER));
		} else {
			$this->debug("Neither _SERVER is defined.");
		}
		...
		if (isset($debug)) {
			...
			$qs = explode('&', $SERVER['QUERY_STRING']);
			foreach ($qs as $v) {
	...
	*/
	function parse_http_headers() {
		...
		} elseif(isset($SERVER) && is_array($SERVER)){
	...
	function register($name,$in=array(),$out=array(),$namespace=false,$soapaction=false,$style=false,$use=false,$documentation='',$encodingStyle=''){
		...
		if(false == $soapaction) {
			if (isset($SERVER)) {
				...
				$SCRIPT_NAME = isset($SERVER['PHP_SELF']) ? $SERVER['PHP_SELF'] : $SERVER['SCRIPT_NAME'];
				$HTTPS = isset($SERVER['HTTPS']) ? $SERVER['HTTPS'] : 'off';
			} else {
