...
namespace cn\eunionz\component\db;
...
class Pdo extends \cn\eunionz\core\Component
{
    ...
    public function exec($sql, $values = null)
    {
        ...
        if (false === $result) {
            throw new \cn\eunionz\exception\DBException($this->getLang('error_db_title'), $this->getLang('error_db_pdo', array($this->get_last_error($_PDOStatement))));
        }
...
//        $this->write_db_logs($pdo, $use_seconds, $values);
    ...
    public function get_fields($table_name)
    {
        ...
        if (empty($result))
            throw new \cn\eunionz\exception\DBException($this->getLang('error_db_title'), $this->getLang('error_db_pdo', array("get `{$table_name}` table fields fial.")));
