 ...
 */
class Pdo extends \cn\eunionz\core\Component
{
    /**
     * 连接超时时间，单位：秒
     * @var int
     ...
     * @var array
     */
    protected static $_query_strs = array();
    ...
    public function init($config = 'db', $cluster_name = 'default')
    {
        ...
        $this->APP_DB_TYPE = $this->getConfig($config, 'APP_DB_TYPE');
        $this->APP_DB_DEPLOY = $this->getConfig($config, 'APP_DB_DEPLOY');
        $this->APP_DB_RW_SEPARATE = $this->getConfig($config, 'APP_DB_RW_SEPARATE');
        $this->APP_DB_PCONNECT = $this->getConfig($config, 'APP_DB_PCONNECT');
        $this->APP_DB_SELECT_LOG_ENABLED = $this->getConfig($config, 'APP_DB_SELECT_LOG_ENABLED');
        $this->APP_DB_SELECT_LOG_TABLE_NAME = $this->getConfig($config, 'APP_DB_SELECT_LOG_TABLE_NAME');
        $this->APP_DB_CHAR = $this->getConfig($config, 'APP_DB_CHAR');
        $this->APP_DB_SERVERS = $this->getConfig($config, 'APP_DB_SERVERS');
        $this->APP_DB_STRUCTURE_CACHE = $this->getConfig($config, 'APP_DB_STRUCTURE_CACHE');
        $this->APP_DB_DATA_STATEMENT_CACHE = $this->getConfig($config, 'APP_DB_DATA_STATEMENT_CACHE');
        $this->APP_DB_DATA_STATEMENT_CACHE_EXPIRES = $this->getConfig($config, 'APP_DB_DATA_STATEMENT_CACHE_EXPIRES');
...
//                    }
                    ...
                    try {
                        $db_link = new \Swoole\Coroutine\MySQL();
                        ...
                        $rs = $db_link->connect([
                            ...
                            'charset' => $this->APP_DB_CHAR,
                            'timeout' => $this->timeout,
                        ]);

                        if (!$rs) {
                            throw new \cn\eunionz\exception\DBException($this->getLang('error_db_title'), $this->getLang('error_db_pdo', array($config['HOST'] . ':' . $config['PORT'], ' connect fail.')));
                        }
                    } catch (\PDOException $e) {
                        throw new \cn\eunionz\exception\DBException($this->getLang('error_db_title'), $this->getLang('error_db_pdo', array($e->getMessage())));
                    }
...
//                        }
                        ...
                        try {
                            ...
                            $rs = $db_link->connect([
                                ...
                                'password' => $slave_config['PASS'],
                                'database' => $slave_config['NAME'],
                                'charset' => $APP_DB_TYPE,
                                'timeout' => $this->timeout,
                            ]);
                            if (!$rs) {
    ...
    public function exec($sql, $values = null)
    {
        ...
        self::$_query_strs[$this->curr_cluster_name] = array(
            'sql' => $sql,
            'params' => $values,
        );
        ...
        if (!$_PDOStatement) {
            throw new \cn\eunionz\exception\DBException($this->getLang('error_db_title'), $this->getLang('error_db_pdo', array("PDO Statement error.")));
        }
        ...
        if (false === $result) {
            throw new \cn\eunionz\exception\DBException($this->getLang('error_db_title'), $this->getLang('error_db_pdo', array($this->get_last_error($pdo))));
        }
...
//        $this->write_db_logs($pdo, $use_seconds, $values);
    ...
    private function write_db_logs($pdo, $use_seconds, $values)
    {
        if ($this->APP_DB_SELECT_LOG_ENABLED && $this->APP_DB_SELECT_LOG_TABLE_NAME) {
            try {
                $shop_id = $this->getConfig('app', 'SHOP_ID');
    ...
    public function commit()
    {
        ...
        if ($this->_trans_times[$this->curr_cluster_name] <= 0) {
            ...
            $this->_trans_times[$this->curr_cluster_name] = 0;
            if (!$result) {
                throw new \cn\eunionz\exception\DBException($this->getLang('error_db_title'), $this->getLang('error_db_pdo', array("tran commit fail.")));
                return false;
            }
        }
        return true;
    ...
    public function rollback()
    {
        ...
        if ($this->_trans_times[$this->curr_cluster_name] <= 0) {
            ...
            if (!$result) {
                throw new \cn\eunionz\exception\DBException($this->getLang('error_db_title'), $this->getLang('error_db_pdo', array("tran rollback fail.")));
                return false;
            }
        } else {
    ...
    {
        if (empty($table_name)) {
            throw new \cn\eunionz\exception\DBException($this->getLang('error_db_title'), $this->getLang('error_db_pdo_table'));
            return false;
        }
        $pdo = $this->getConnect();
        if (!$pdo) {
        ...
        if ($result) {
            foreach ($result as $val) {
                ...
                if (!isset($fields['primary']))
                    throw new \cn\eunionz\exception\DBException($this->getLang('error_db_title'), $this->getLang('error_db_pdo_table_primary', array($table_name)));
    ...
    public function get_tables()
    {
        ...
        foreach ($result as $row) {
            foreach ($row as $table) {
                if ($table && !in_array($table, $tables)) $tables[] = $table;
            }
    ...
    public function get_sql($params = null)
    {
        if (empty($params) || !is_array($params)) {
            if (!isset(self::$_query_strs[$this->curr_cluster_name])) {
                return "";
            }
            $params = self::$_query_strs[$this->curr_cluster_name];
     ...
     * @return string
     */
    public function get_last_error($_PDO)
    {
        return $_PDO->error;
    }
    ...
    public function xa_start($xa_name = null)
    {
        ...
        $_PDOStatement = $pdo->prepare($sql);
        if (!$_PDOStatement) {
            throw new \cn\eunionz\exception\DBException($this->getLang('error_db_title'), $this->getLang('error_db_pdo', array("XA PDO Statement error.")));
        }
        ctx()->setExecuteQuerys(ctx()->getExecuteQuerys() + 1);
