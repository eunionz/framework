...
namespace com\eunionz\component\upload;
...
class Upload extends \com\eunionz\core\Component
{
	// 实例
	...
	public function move($data, $savePath, $nameRules = null, $extension = null, $maxSize = null,$prompt_type="js")
	{
		...
		foreach ( $data as $file )
		{
			...
			if (empty($file['name']))
			{
                throw new \com\eunionz\exception\UploadException($this->getLang('error_upload_title'),$this->getLang('error_upload_filename',array($file['name'])));
			}
    ...
    public function moves($data, $savePath, $nameRules = null, $extension = null, $maxSize = null)
    {
        ...
        if (empty($data) || !is_array($data))
        {
            throw new \com\eunionz\exception\UploadException($this->getLang('error_upload_title'),$this->getLang('error_upload_files'));
        }
	...
	private function save($file)
	{
		...
		{
            throw new \com\eunionz\exception\UploadException($this->getLang('error_upload_title'),$this->getLang('error_upload_save'));
		}
	...
	private function checkExt($ext, $extension,$prompt_type="js")
	{
		if (!empty($extension))
		{
			...
			if (!in_array(strtolower($ext), $extension))
			{
                ...
                //throw new \com\eunionz\exception\UploadException($this->getLang('error_upload_title'),$this->getLang('error_upload_extension',array($ext)));
			}
		}
	...
	private function checkSize($size, $maxSize)
	{
		if (!empty($maxSize))
		{
			if ($size > $maxSize)
			{
                throw new \com\eunionz\exception\UploadException($this->getLang('error_upload_title'),$this->getLang('error_upload_filesize',array(ceil($maxSize / 1024))));
			}
		}
	...
	private function checkUpload($filename)
	{
		...
		{
            throw new \com\eunionz\exception\UploadException($this->getLang('error_upload_title'),$this->getLang('error_upload_mode'));
		}
	...
	private function getDirectory($savePath)
	{
		if (!is_dir($savePath))
		{
			...
			{
                throw new \com\eunionz\exception\UploadException($this->getLang('error_upload_title'),$this->getLang('error_upload_mkdir',array($savePath)));
			}
		}
		
		if (!is_writable($savePath))
		{
            throw new \com\eunionz\exception\UploadException($this->getLang('error_upload_title'),$this->getLang('error_upload_dir_write',array($savePath)));
		}
	...
	private function getErrorCode($errorNo)
	{
		switch ($errorNo)
		{
			...
			case 3 :
			case 4 :
                throw new \com\eunionz\exception\UploadException($this->getLang('error_upload_title'),$this->getLang('error_upload_' . $errorNo . '_code'));
				break;
			case 6 :
                throw new \com\eunionz\exception\UploadException($this->getLang('error_upload_title'),$this->getLang('error_upload_' . $errorNo . '_code'));
                break;
			case 7 :
				...
				break;
			default :
                throw new \com\eunionz\exception\UploadException($this->getLang('error_upload_title'),$this->getLang('error_upload_0_code'));
