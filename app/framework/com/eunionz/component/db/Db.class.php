...
use com\eunionz\exception\IOException;
...
class Db extends Pdo
{
            ...
            if ('[null]' == strtolower($value)) {
                ...
                foreach ($exp_fields_new as $_field) {
                    ...
                    if (!($field_tmp = $this->parse_check_field($_field))) {
                        throw new \com\eunionz\exception\ModelException($this->getLang('error_model_title'), $this->getLang('error_model_field_not_exist', array($_field)));
                    }
    ...
    public function parse_check_table($table)
    {
        ...
        if (!in_array($table, $this->_tables[$this->curr_cluster_name]))
            throw new \com\eunionz\exception\ModelException($this->getLang('error_model_title'), $this->getLang('error_model_table_not_exist', array($table)));
    ...
    {
        if (empty($tables))
            throw new \com\eunionz\exception\ModelException($this->getLang('error_model_title'), $this->getLang('error_model_table_lack'));
    ...
    public function parse_distinct(array $fields)
    {
        ...
        foreach ($fields as $field) {
            ...
            if (!($new_field = $this->parse_check_field($field)))
                throw new \com\eunionz\exception\ModelException($this->getLang('error_model_title'), $this->getLang('error_model_field_not_exist', array($field)));
    ...
    public function parseJoin($expression)
    {
        ...
        foreach ($expression as $table => $join) {
            ...
            if (!is_array($join) || empty($join))
                throw new \com\eunionz\exception\ModelException($this->getLang('error_model_title'), $this->getLang('error_model_join'));
            ...
            foreach ($join as $field => $KV) {
                ...
                if (is_integer($field)) {
                    //如果是整数，表示同一张表连接多次
                    if (!is_array($KV))
                        throw new \com\eunionz\exception\ModelException($this->getLang('error_model_title'), $this->getLang('error_model_must_array'));
    ...
    public function parse_check_field($field, $cur_table = null, $_alias = NULL)
    {
        ...
        if (stripos($field, '.')) {
            ...
            if (!in_array($field, $this->_fields[$this->curr_cluster_name][$table]['fields']))
                throw new \com\eunionz\exception\ModelException($this->getLang('error_model_title'), $this->getLang('error_model_table_field_not_exist', array($field, $table)));
            if ($alias) {
                $field_result = $this->set_special_char($alias) . '.' . $this->set_special_char($field);
            } else {
            ...
            if (!empty($cur_table)) {
                ...
                if (!in_array($cur_table, $tables))
                    throw new \com\eunionz\exception\ModelException($this->getLang('error_model_title'), $this->getLang('error_model_table_not_exist', array($cur_table)));
...
//        }
            ...
            throw new \com\eunionz\exception\ModelException($this->getLang('error_model_title'), $this->getLang('error_model_parse'));
        }
                        ...
                        if (in_array($field_key, $symbol)) {
                            ...
                            if (in_array($field_key, array('IN', 'NOT IN'))) {
                                ...
                                if (!is_array($field_val) || (count($field_val) != 2))
                                    throw new \com\eunionz\exception\ModelException($this->getLang('error_model_title'), $this->getLang('error_model_between', array($field)));
...
//                                $field_val = trim("'", $this->parse_value($field_val));
    ...
    public function parse_group(array $groups)
    {
        ...
        foreach ($groups as $group) {
            ...
            if (!($new_group = $this->parse_check_field($group)))
                throw new \com\eunionz\exception\ModelException($this->getLang('error_model_title'), $this->getLang('error_model_field_not_exist', array($group)));
    ...
    public function parse_order(array $orders)
    {
        ...
        foreach ($orders as $field => $orderStr) {
            ...
            if (!in_array($orderStr, array('DESC', 'ASC')))
                throw new \com\eunionz\exception\ModelException($this->getLang('error_model_title'), $this->getLang('error_model_orderby'));
    ...
    /**
     * 解析添加语句，并返回数组，数组格式为：array("sql"=>"... :field ...","params"=>array(":field"=>value,...))
     * @param    mixed $expression 字段值数组，格式：array("field1"=>value1,...)
     * @param    string $table 表名，必须
     * @return    array("sql"=>"... :field ...","params"=>array(":field"=>value,...))
     */
    ...
    public function update($tablename, array $data, array $options, $return_sql = false)
    {
        ...
        // 解析值
        $_setStr = $this->parse_update($data);
