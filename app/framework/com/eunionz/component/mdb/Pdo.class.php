...
namespace com\eunionz\component\mdb;
 ...
 * PDO 数据库操作抽象类
 */
abstract class Pdo extends \com\eunionz\core\Component
{
    /**
    ...
    */
    ...
    public function get_tables($is_master=true)
    {
        ...
        if (false === $result)
            throw new \com\eunionz\exception\DBException($this->getLang('error_db_title'),$this->getLang('error_db_pdo',array($this->get_last_error())));
    ...
    public function exec($sql,$values=null,$tables=array())
    {
        ...
        if(!$this->cache_settings){
            throw new \com\eunionz\exception\DBException($this->getLang('error_db_title'),$this->getLang('error_db_pdo_cache_config'));
        }
