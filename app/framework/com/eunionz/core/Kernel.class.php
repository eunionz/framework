...
";
    ...
    public final function loadCore($class, $single = true)
    {
        if (strpos($class, '\\') === false) {
            $class = "\\com\\eunionz\\core\\" . ucfirst($class);
        }
    ...
    public function loadComponent($class, $single = true)
    {
        if (false === stripos($class, "\\")) {
            $class = "\\com\\eunionz\\component\\" . strtolower($class) . '\\' . ucfirst($class);
        }
    ...
    public function loadActionByUrl($url, $get = array(), $post = array(), $files = array(), $request = array())
    {
        ...
        if (!method_exists($controller, '_' . $action)) {
            ...
            throw new \com\eunionz\exception\MethodNotFoundException(ctx()->getI18n()->getLang('error_router_title'), ctx()->getI18n()->getLang('error_router_controller_miss_method', array($classes, '_' . $action)));
        }
    ...
    public static function getConfig($names, $key = '')
    {
        ...
        if (!isset(self::$_app_config_types[$names])) {
            ...
            if (!$local_config_file && !$global_config_file) {
                $i18n = new I18n(null);
                throw new \com\eunionz\exception\FileNotFoundException($i18n->getLang('error_config_file_not_found', array($names)));
            }
     ...
     * @throws \com\eunionz\exception\BaseException
     */
