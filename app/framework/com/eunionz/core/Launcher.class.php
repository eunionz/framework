 ...
 */
class Launcher extends \com\eunionz\core\Kernel
{
            ...
            if ($server_index) {
                ...
                try {
                    ...
                    if (empty($server_cfgs) || !is_array($server_cfgs)) {
                        throw new \com\eunionz\exception\BaseException($i18n->getLang('error_server_config'));
                    }
                    if (!isset($server_cfgs[$server_index])) {
                        throw new \com\eunionz\exception\BaseException($i18n->getLang('error_main_server_config'));
                    }
                    ...
                    if (strtolower($main_server_cfg['server_type']) == 'http') {
                        ...
                        if (!self::$_swoole_main_server) {
                            throw new \com\eunionz\exception\BaseException($i18n->getLang('error_main_server_create_failure'));
                        }
                    ...
                    if (isset($main_server_cfg['on_start']) && $main_server_cfg['on_start']) {
                        self::$_swoole_main_server->on("start", function ($server) use ($main_server_cfg, $i18n) {
                            ...
                            if (!method_exists($object, $method)) {
                                throw new \com\eunionz\exception\MethodNotFoundException($i18n->getLang('error_hook_method_not_found_title'), $i18n->getLang('error_hook_method_not_found', array($class, $method)));
                            }
